...
import { Clock, AlertTriangle, CheckCircle2, Pause, Play } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      <div className="space-y-2">
        {workItems.slice(0, 5).map((item, i) => {
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 mt-1">
                  <span className="text-xs text-text-muted">{item.client}</span>
              ...
              {/* Status Badge */}
              <div className="flex items-center gap-2">
                <span className="text-xs text-text-muted hidden lg:block">Due {new Date(item.dueDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span>
                <div className={clsx('flex items-center gap-1 px-2 py-1 rounded-full text-[10px] font-semibold', `${status.textColor} bg-${status.textColor.replace('text-', '')}/10`)}>
