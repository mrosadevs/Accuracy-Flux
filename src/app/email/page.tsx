  ...
  return (
    <div className="relative group/thread">
      <button onClick={onClick}
        className={clsx("w-full text-left px-3 py-3 rounded-xl transition-all group pr-8", active ? "bg-primary-50 border border-primary-200" : "hover:bg-surface-hover border border-transparent")}>
      ...
      <div className="absolute top-2 right-2">
        <button
          ...
          className="p-1 rounded-lg hover:bg-surface-hover opacity-0 group-hover/thread:opacity-100 transition-opacity z-10"
        >
                ...
                <ThreadItem key={thread.id} thread={thread} active={selectedThreadId === thread.id}
                  ...
                  onRename={async () => {
                    ...
                    if (newTitle && newTitle.trim() && newTitle.trim() !== thread.title) {
                      try { await renameThread(thread.id, newTitle.trim()); } catch (e) { console.error(e); }
                    }
                  }} />
              ))
            ) : (
